 ...
 * This example demonstrates using only RPC (HTTP) without WebSocket subscriptions,
 * which may be useful in environments where WebSocket connections are not available
 * or desired.
 *
 ...
 *
 * To run this example, execute `pnpm start:rpc` in this directory.
 */
...
const SOURCE_ACCOUNT_SIGNER = await createKeyPairSignerFromBytes(
     ...
     */
    new Uint8Array([
        2, 194, 94, 194, 31, 15, 34, 248, 159, 9, 59, 156, 194, 152, 79, 148, 81, 17, 63, 53, 245, 175, 37, 0, 134, 90,
        111, 236, 245, 160, 3, 50, 196, 59, 123, 60, 59, 151, 65, 255, 27, 247, 241, 230, 52, 54, 143, 136, 108, 160, 7,
        128, 4, 14, 232, 119, 234, 61, 47, 158, 9, 241, 48, 140,
 ...
 *
 * Note: This example uses only HTTP RPC without WebSocket subscriptions, relying on polling for
 ...
 */
const sendAndPollToConfirmTransaction = sendAndPollToConfirmTransactionFactory({
    /**
     * The RPC implements a `sendTransaction` method which relays transactions to the network.
     * Transaction confirmation will be handled through polling the RPC endpoint.
     */
    rpc,
    pollingInterval: 1000, // Optional: controls how frequently to poll for transaction status
});
...
try {
    await sendAndPollToConfirmTransaction(signedTransaction, { commitment: 'confirmed' });
    log.info('[success] Transfer confirmed via polling');
